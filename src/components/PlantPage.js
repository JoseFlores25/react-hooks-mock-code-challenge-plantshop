...
function PlantPage() {
  ...
  plantsToDisplay = plantsToDisplay.filter(
    (plant) =>
      plant.name.includes(search) ||
      plant.name.toLowerCase().includes(search) ||
      plant.name.toUpperCase().includes(search)
  );
  ...
  function onAddPlant(newPlant) {
    ...
    const indexOfPlant = plants.findIndex(
      (plantInArray) => plantInArray.id === newPlant.id
    );
    ...
    if (indexOfPlant !== -1) {
      ...
      fetch(`http://localhost:3003/plants/${newPlant.id}`, {
        method: "PATCH",
        headers: {
